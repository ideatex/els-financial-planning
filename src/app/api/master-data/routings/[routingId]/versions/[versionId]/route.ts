...
import { updateRoutingVersionStatus } from '@/server/services/routing.service';
...
export const PATCH = createApiHandler(
  async (req: NextRequest, { params }: { params: Record<string, string | string[]> }) => {
    ...
    const version = await updateRoutingVersionStatus(context.organization.id, context.user.id, versionId, body);
