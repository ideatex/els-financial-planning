...
import { updateBomVersionStatus } from '@/server/services/bom.service';
...
export const PATCH = createApiHandler(
  async (req: NextRequest, { params }: { params: Record<string, string | string[]> }) => {
    ...
    const version = await updateBomVersionStatus(context.organization.id, context.user.id, versionId, body);
